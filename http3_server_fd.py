...
class HttpServerProtocol(QuicConnectionProtocol):
    ...
    def http_event_received(self, event: H3Event) -> None:
       # cic.send_data(file_name='/root/small_file1.png')
    ...
    def quic_event_received(self, event: QuicEvent):
        if isinstance(event, StreamDataReceived):
            if self.f is None:
                ...
                self.f = open("10Gigfile1.pdf", 'w+b')
...
'''class ImgServerProtocol(QuicConnectionProtocol):
    ...
    def quic_event_received(self, event: QuicEvent):
        if isinstance(event, StreamDataReceived):
            if self.f is None:
                print("Opening for the first time")
                self.f = open("/root/big_file.png", 'w+b')
...
class ImgClient(QuicConnectionProtocol):
   # def __init__(self, *args, **kwargs):
    ...
    def send_data(self, file_name) -> None:
        ...
        stream_id = server._quic.get_next_available_stream_id()
        stream_end = False
...
if __name__ == "__main__":
   ...
   # cisp = ImgServerProtocol(quic=configuration)
