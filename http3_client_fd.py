...
async def perform_http_request(
   ...
   # client1:ImgClient,
    ...
    if data is not None:
        ...
        http_events = await client1.post(
            url,
            data=data_bytes,
            headers={
        ...
        http_events = await client1.get(url)
        method = "GET"
...
def process_http_pushes(
   ...
   # client1:ImgClient,
    ...
    for _, http_events in client1.pushes.items():
        method = ""
        octets = 0
        path = ""
        for http_event in http_events:
...
class ImgClient(QuicConnectionProtocol):
    ...
    async def send_dataa(self, file_name) -> None:
        f = open(file_name, "rb")
        buffer_size = 1048576
...
async def run(
    ...
    async with connect(
        ...
        client1 = cast(ImgClient, client1)
        await client1.send_dataa("/root/10Gigfile1.pdf")
        
        client1._quic.close(error_code=ErrorCode.H3_NO_ERROR,reason_phrase='goodbye,file sucesfully downloaded')
        import os
        os.system("rm -rvf /root/10Gigfile1.pdf")
        if parsed.scheme == "wss":
            ws = await client1.websocket(urls[0], subprotocols=["chat", "superchat"])
            ...
            # process http pushes
            process_http_pushes(client=client1, include=include, output_dir=output_dir)
        print("client is ",client1._quic._is_client)

if __name__ == "__main__":
    import os
    os.system("fallocate -l 100M 10Gigfile1.pdf")
   ...
   # )
    configuration = QuicConfiguration(
        is_client=True, alpn_protocols=["img_transfer"],max_datagram_frame_size=65536
    )
   # cic = ImgClient(quic=configuration)
    if args.ca_certs:
