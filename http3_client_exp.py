...
aioquic.quic.connection.QuicConnection.host_cid = binascii.unhexlify("0011223344556677")

# aioquic.quic.connection.QuicConnection.datagrams_to_send(now=time.time())

from aioquic.quic.connection import QuicConnectionId

aioquic.quic.connection.QuicConnection._peer_cid = QuicConnectionId(
...
class HttpClient(QuicConnectionProtocol):
    ...
    async def _request(self, request: HttpRequest) -> Deque[H3Event]:
        ...
        stream_id = self._quic.get_next_available_stream_id()
        rsid=int(stream_id)
        print("next available stream id is",stream_id)
...
async def perform_http_request(
    ...
    # perform request
   # import binascii
    #client._quic.host_cid = binascii.unhexlify("0011223344556677")

    #client._quic.datagrams_to_send(now=time.time())

    #from aioquic.quic.connection import QuicConnectionId
    #client._quic._peer_cid = QuicConnectionId(
     #   cid=binascii.unhexlify("0011223344556677"), sequence_number=None
    #)
...
async def run(
    ...
    async with connect(
        ...
        client._quic._on_new_connection_id_delivery(QuicDeliveryState.ACKED,connection_id=2)
        client._quic.datagrams_to_send(now=time.time())
        client._quic._on_retire_connection_id_delivery(QuicDeliveryState.LOST, 0)
        client._quic.datagrams_to_send(now=time.time())
        print("currently used sid is",rsid)
        sid=client._quic.get_next_available_stream_id()
        print("code is here and sid is",sid)
        intsid=int(sid)
        intz=int(0)
        #sidh3=client._quic._get_or_create_stream(frame_type=FrameType.WEBTRANSPORT_STREAM,stream_id=intsid)
        #sidh3=client._quic._get_or_create_stream()
       # sids = client._quic._get_or_create_stream_for_send(stream_id=isid)
        client._quic.reset_stream(stream_id=rsid,error_code=QuicErrorCode.NO_ERROR)
        #client._quic.send_stream_data(stream_id=rsid,data=b'hai')
        client._quic.send_stream_data(stream_id=intsid,data=b'hello')
        #client._quic.send_stream_data(stream_id=sidh3, data=b'hello')
        #client._quic.send_stream_data(stream_id=sids, data=b'hello')
        #client._quic.reset_stream(stream_id=rsid,error_code=QuicErrorCode.NO_ERROR)
        sidn=client._quic.get_next_available_stream_id()
        print("code is here and reset done and  new sid is",sidn)
        from aioquic.quic.stream import QuicStreamSender
       # QuicStreamSender.reset(error_code=QuicErrorCode.NO_ERROR)
        p=QuicStreamSender(stream_id=intsid,writable=True)
        p.reset(error_code=ErrorCode.H3_NO_ERROR)
        q=QuicStreamSender(stream_id=sidn,writable=True)
        q.write(data=b'how are you')
        client._quic.send_stream_data(stream_id=sidn,data=b'hi')
        import binascii 

#        from aioquic.quic.packet import encode_quic_retry
 #       import binascii
  #      client._quic.receive_datagram(data=
   #         encode_quic_retry(
 ...
 #           addr="172.16.2.2",
  #          now=time.time(),
   #     ) 
        ...
        client._quic.original_destination_connection_id
       # client._quic.change_connection_id()
