...
async def run(
    ...
    async with connect(
        ...
        def _mysidrun2():
            mysid=int(123)
            client._quic._get_or_create_stream_for_send(stream_id=mysid) #create our own stream
            client._quic.send_stream_data(stream_id=mysid, data=b'this is my stream,multi') #send data to our own stream
           # client._quic.reset_stream(stream_id=mysid, error_code=QuicErrorCode.NO_ERROR) # do reset of our own stream
            # client._quic.send_stream_data(stream_id=mysid, data=b'again send data to reset') #proof to reset is done
        # from threading import *
        ...
        def _mysidrun3():
            mysid=int(1234)
            client._quic._get_or_create_stream_for_send(stream_id=mysid) #create our own stream
            client._quic.send_stream_data(stream_id=mysid, data=b'this is my stream,multi sid') #send data to our own stream
           # client._quic.reset_stream(stream_id=mysid, error_code=QuicErrorCode.NO_ERROR) # do reset of our own stream
            # client._quic.send_stream_data(stream_id=mysid, data=b'again send data to reset') #proof to reset is done
        # from threading import *
       ...
       # t2 = Thread(target=_mysidrun2)
       # t3 = Thread(target=_mysidrun3)
       # t1.start()
       # t2.start()
       # t3.start()
       # t1.join()
       ...
       # def close_conn():
        #    time.sleep(0.1)
        #    client._quic.close(error_code=ErrorCode.H3_NO_ERROR,reason_phrase="goodbye")
       ...
       # client._quic._on_retire_connection_id_delivery(QuicDeliveryState.LOST,0)
        ...
        ''' 
        from aioquic.quic.connection import QuicConnectionId, QuicConnection, QuicReceiveContext
        from aioquic import tls
        def some_name(client, epoch=tls.Epoch.ONE_RTT):
        ...
        def close_conn():
           # time.sleep(0.1)
            ...
            mysid=int(1234)
            client._quic._get_or_create_stream_for_send(stream_id=mysid) #create our own stream
            client._quic.send_stream_data(stream_id=mysid, data=b'this is my stream,multi sid')
            ...
            builder = QuicPacketBuilder(
                host_cid=client._quic.host_cid,
                is_client=True,
                peer_cid=client._quic._peer_cid.cid,
          ...
          # crypto.setup_initial(client.host_cid, is_client=True, version=client._version)
          # builder.start_packet(PACKET_TYPE_INITIAL, crypto)
            client._quic._write_connection_close_frame(
                builder=builder,
