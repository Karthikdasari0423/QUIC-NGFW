...
async def test_max_uni_streams_kdaquic(server: Server, configuration: QuicConfiguration):
    ...
    async with connect(
        ...
        max_uni_streams_server_limit = quic_conn.remote_transport_parameters.initial_max_streams_uni
        ...
        for i in range(int(max_uni_streams_server_limit) + 5): # Try to open a few more than the limit
            ...
            if quic_conn._streams_uni_opened < quic_conn.remote_transport_parameters.initial_max_streams_uni:
                ...
                print(f"Attempting to open unidirectional stream ID: {stream_id_to_create} ({quic_conn._streams_uni_opened + 1}/{quic_conn.remote_transport_parameters.initial_max_streams_uni})")
                try:
                ...
                print(f"Reached server's unidirectional stream limit ({quic_conn.remote_transport_parameters.initial_max_streams_uni}). "
        ...
        if opened_streams_count == quic_conn.remote_transport_parameters.initial_max_streams_uni:
            print(f"Successfully opened and used all {quic_conn.remote_transport_parameters.initial_max_streams_uni} unidirectional streams.")
            server.result |= Result.M
        elif opened_streams_count < quic_conn.remote_transport_parameters.initial_max_streams_uni :
            print(f"Warning: Opened only {opened_streams_count} out of {quic_conn.remote_transport_parameters.initial_max_streams_uni} streams before loop ended or error.")
            ...
            # due to the check `quic_conn._streams_uni_opened < quic_conn.remote_transport_parameters.initial_max_streams_uni`.
            print(f"Error: Opened {opened_streams_count} streams, which is more than the limit {quic_conn.remote_transport_parameters.initial_max_streams_uni}.")
