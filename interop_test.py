...
async def test_receive_datagram_retry(server: Server, configuration: QuicConfiguration):
    async with connect(
            server.host, server.port, configuration=configuration
    ) as protocol:

        from aioquic._buffer import Buffer
        from aioquic.quic.packet import QuicErrorCode, RETRY_INTEGRITY_TAG_SIZE, PACKET_TYPE_RETRY, \
            get_retry_integrity_tag
        def drop(sender):
            return len(sender._quic.datagrams_to_send(now=time.time()))
        ...
        def encode_quic_retry(
            ...
            return buf.data
        import binascii
        protocol._quic.receive_datagram(
            encode_quic_retry(
                version=protocol._quic._version,
                source_cid=binascii.unhexlify("85abb547bf28be97"),
                destination_cid=protocol._quic.host_cid,
                original_destination_cid=protocol._quic._peer_cid.cid,
                retry_token=bytes(16),
            ),
            addr=server.host,
            now=time.time(),
        ...
        if drop(protocol) == 1:
            print("drop client ",drop(protocol))
            server.result |= Result.M
        else:
...
async def test_version_negotiation_fail(server: Server, configuration: QuicConfiguration):
    async with connect(
            server.host, server.port, configuration=configuration
    ) as protocol:
        import os
        ...
        def encode_quic_version_negotiation(
            ...
            from aioquic.quic.packet import PACKET_LONG_HEADER
        ...
        protocol._quic.receive_datagram(
            encode_quic_version_negotiation(
                source_cid=protocol._quic._peer_cid.cid,
                destination_cid=protocol._quic.host_cid,
                supported_versions=[0xFF001122],
            ),
            addr=server.host,
            now=time.time(),
        )
...
async def test_version_negotiation_wrong_integrity_tag(server: Server, configuration: QuicConfiguration):
    async with connect(
            server.host, server.port, configuration=configuration
    ) as protocol:
        from aioquic._buffer import Buffer
...
async def test_pull_retry(server: Server, configuration: QuicConfiguration):
    async with connect(
            server.host, server.port, configuration=configuration
    ) as protocol:
        import os
        os.system("cp /root/aioquic/tests/retry.bin /root/aioquic/examples/")
        token = b"F\xb2\xe0-EH\x0b\xa6d>\\n}H\xec\xb4"
        from aioquic._buffer import Buffer
        from aioquic.quic.packet import QuicErrorCode, RETRY_INTEGRITY_TAG_SIZE, PACKET_TYPE_RETRY, \
            get_retry_integrity_tag
...
async def test_pull_retry_draft_29(server: Server, configuration: QuicConfiguration):
    async with connect(
            server.host, server.port, configuration=configuration
    ) as protocol:
        import os
        os.system("cp /root/aioquic/tests/retry_draft_29.bin /root/aioquic/examples/")
